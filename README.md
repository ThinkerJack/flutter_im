...
也推荐做本地存储，接收流程和消息表（chat_detail）流程类似，不过维护的是消息列表（chat_list）表。

![](https://img-blog.csdnimg.cn/2021101518550635.png)

当收到消息时，需要先判断本地消息列表是否有当前消息用户的对话框，如果没有就先插入，有就更新。

更新时主要关注的数据为本地数据库chatlist表中的unreadCount数据，用来显示未读消息的数量，同时也要更新最后一句聊天内容和聊天时间。

当用户点击消息列表，进入聊天画面，并从聊天画面退出时，需要将数据库chatlist表中的unreadCount清零，同时也要更新最后一句聊天内容和聊天时间。
...
- 聊天页消息的时间逻辑
  ...
  第一个原则是同一年/月/日的消息，第二条消息与上条消息相同的年/月/日就不展示。

  第二个原则是间隔五分钟以内的消息不展示。

  根据以上两个原则，我们就可以设计我们的聊天时间逻辑页面。

  首先当前消息页面的最后一条消息日期是一定要展示的。

  第二条消息的时间是否展示就根据第二条消息和前一条消息的时间间隔来判断，以此类推。
...
- flutter页面刷新：当收到socket消息，除了更新本地数据库之外，我们还要通知flutter页面刷新，借助不同的状态管理方案即可实现，demo中使用了flutter自带的`ChangeNotifier`。
